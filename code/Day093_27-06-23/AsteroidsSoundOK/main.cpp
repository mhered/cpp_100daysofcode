...
private:
	...
	// ship models
	std::vector<std::pair<float, float>> model_ship, model_ship_nothrust, model_ship_thrust, model_ship_destroyed;
	// asteroid model
	std::vector<std::pair<float, float>> model_asteroid;

	// Declare sound engine and sound effects
	olc::sound::WaveEngine engine;							  // manages devicesand plays sounds
	olc::sound::Wave fire, boom, explosion, thrust, gameover; // sample waveforms
...
protected:
	...
	bool OnUserCreate() override
	{
		...
		std::string path = "/home/mhered/cpp_100daysofcode/code/Day093_27-06-23/AsteroidsSoundOK/soundclips/";

		fire.LoadAudioWaveform(path + "fire.wav");
		boom.LoadAudioWaveform(path + "boom.wav");
		explosion.LoadAudioWaveform(path + "explosion.wav");
		thrust.LoadAudioWaveform(path + "thrust-xs.wav");
		gameover.LoadAudioWaveform(path + "gameover.wav");
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		if (is_ship_dead)
		{
			engine.PlayWaveform(&explosion, false, 1.0f);
			sleep(0.4f);
			engine.PlayWaveform(&gameover, false, 2.0f);
			sleep(2.0f);
			ResetGame(0);
		}
		...
		if (GetKey(olc::Key::UP).bHeld)
		{
			...
			// play thrust sound
			engine.PlayWaveform(&thrust, false, 4.0);
		}
		else
		...
		for (auto &a : vecAsteroids)
		{
			...
			{
				is_ship_dead = true;
				model_ship = model_ship_destroyed;
			}
		}
